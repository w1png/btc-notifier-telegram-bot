...
if __name__ == "__main__":
    ...
    parser.add_argument("-b", "--borderwidth", help="border width of the graph.")

    if parser.parse_args().debug_mode:
        settings = Settings()
        debug_mode = parser.parse_args().debug_mode
        if debug_mode in ["True", "False"]:
            print(f"Debug mode was updated from \"{settings.get_debug_mode()}\" to \"{debug_mode}\"")
    ...
    if parser.parse_args().color:
        ...
        if re.search(r'^#(?:[0-9a-fA-F]{3}){1,2}$', color): # No idea how this works, taken from https://stackoverflow.com/questions/30241375/python-how-to-check-if-string-is-a-hex-color-code
            print(f"Graph color was changed from \"{settings.get_graphcolor()}\" to \"{color}\".")
            settings.set_graphcolor(f"\"color\"")
        else:
            print(f"\"{color}\" is not a valid hex color.", parser.print_help(), sep="\n")
    if parser.parse_args().border:
        settings = Settings()
        border = parser.parse_args().border
        if border.isalnum():
            print(f"Border width was changed from \"{settings.get_border()}\" to \"{border}\".")
            settings.set_border(border)
        else:
            print(f"\"{border}\" is not a number!", parser.print_help(), sep="\n")
